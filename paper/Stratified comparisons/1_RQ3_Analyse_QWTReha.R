...
match.data <- read.csv("Data/preprocessed data/20240611_RQ3_QWT-Reha_MatchedData.csv")
...
desc <- data %>%
  ...
  summarise(age.aud.mean = wtd.mean(age.aud, weights), age.aud.std = sqrt(wtd.var(age.aud, weights)),
            ...
            time.aud.qwt.median = weighted.median(time.aud.qwt, weights), 
            time.aud.qwt.iqr1 = weighted.quantile(time.aud.qwt, weights, probs = 0.25),
            time.aud.qwt.iqr3 = weighted.quantile(time.aud.qwt, weights, probs = 0.75),
            time.qwt.median = weighted.median(time.qwt, weights),
            time.qwt.iqr1 = weighted.quantile(time.qwt, weights, probs = 0.25),
            time.qwt.iqr3 = weighted.quantile(time.qwt, weights, probs = 0.75),
            time.reha.median = weighted.median(time.reha, weights),
            time.reha.iqr1 = weighted.quantile(time.reha, weights, probs = 0.25),
            time.reha.iqr3 = weighted.quantile(time.reha, weights, probs = 0.75),
  ...
  t %>% as.data.frame() %>% row_to_names(1) %>% `colnames<-`(c("QWT", "QWT & Reha"))
  
# Output Table 1
#write.csv(desc, paste0("Output/", DATE, "_Tab1_QWT-Reha.csv"))
...
#ggsave(paste0("Output/", DATE, "_FigA1_QWT-Reha_HospDays.jpg"), dpi=300, width = 10, height = 8)

mHospDays <- zeroinfl(hosp.FU.days ~ as.factor(reha) + time.aud.qwt + time.qwt, data = data, 
...
mHospTimes <- zeroinfl(hosp.FU.times ~ as.factor(reha) + time.aud.qwt + time.qwt, data = data, 
                       dist = "negbin", weights = weights)
summary(mHospTimes)
...
ggplot(data, aes(hosp.FU.days)) + geom_histogram() + 
  ...
  xlab("Days") + theme_barplot
#ggsave(paste0("Output/", DATE, "_FigA1_QWT-Reha_HospAUDDays.jpg"), dpi=300, width = 10, height = 8)

mHospAUDDays <- zeroinfl(hosp.FU.AUD.days ~ as.factor(reha) + time.aud.qwt + time.qwt, data = data, 
                         dist = "negbin", weights = weights)
...
mHospAUDTimes <- zeroinfl(hosp.FU.AUD.times ~ as.factor(reha) , data = data, 
                          dist = "negbin", weights = weights)
...
# Output Table 2
out <- rbind(
  cbind(var = "Hosp_AllCause_Days/Reha/count", 
  ...
  cbind(var = "Hosp_AllCause_Days/TimeQWT/count", 
        coef = exp(out_1[4]), lci = exp(confint(out_1)[4]), uci = exp(confint(out_1)[12]), pval = out_1[28]),
  cbind(var = "Hosp_AllCause_Days/Reha/zero", 
  ...
  cbind(var = "Hosp_AllCause_Days/TimeQWT/zero", 
        coef = exp(out_1[8]), lci = exp(confint(out_1)[8]), uci = exp(confint(out_1)[16]), pval = out_1[32]),
  cbind(var = "Hosp_AUDCause_Days/Reha/count", 
  ...
  cbind(var = "Hosp_AUDCause_Days/TimeQWT/count", 
        coef = exp(out_3[4]), lci = exp(confint(out_3)[4]), uci = exp(confint(out_3)[12]), pval = out_3[28]),
  cbind(var = "Hosp_AUDCause_Days/Reha/zero", 
  ...
  cbind(var = "Hosp_AUDCause_Days/TimeQWT/zero", 
...
# Output Table 2
#write.csv(out, paste0("Output/", DATE, "_Tab2_QWT-Reha.csv"))
